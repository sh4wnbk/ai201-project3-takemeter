...
1. **Annotation pre-labeling (disclosed).** I directed an LLM to draft a label for each of the 270 collected comments using my exact label definitions and decision rules, writing drafts to a separate `label_suggested` column. Most drafts came from Claude `claude-haiku-4-5`; a small number of early rows were drafted by Cerebras before I switched providers (the Cerebras call kept hanging without a timeout). I then **reviewed and corrected every row by hand** against the rules — the pre-labels were never used as final labels. The per-class totals happened to match the drafts (144/80/46), but rows were individually re-judged, including a blind calibration pass on a subset where I labeled without seeing the draft first. I overrode roughly _[N]_ drafts during review.
